...
var TT = TAOTAO = {
	// 编辑器参数
	kingEditorParams : {
		filePostName  : "uploadFile", // 图片上传时表单提交input的名称
    ...
    initItemCat : function(data){
    	$(".selectItemCat").each(function(i,e){
    		var _ele = $(e);
    		if(data && data.itemCatName){
    			//_ele.after("<span style='margin-left:10px;'>"+data.cid+"</span>");
    			_ele.after("<span style='margin-left:10px;'>"+data.itemCatName+"</span>");
    		}else{
    			    	...
    			    	$("ul",_win).tree({
    			    		...
    			    		onClick : function(node){
    			    			if($(this).tree("isLeaf",node.target)){
    			    				...
    			    				if(data && data.fun){
										// 执行传入的方法
										// 参数：第一个参数一般固定为this(作用域),从第二参数开始，是定义方法中的参数
